...
while pq:
    ...
    if (y, x) != (0, 0) and consecutive < 10 and 0 <= y + dy < len(grid) and 0 <= x + dx < len(grid[0]):
        heapq.heappush(pq, (loss + grid[y + dy][x + dx], y + dy, x + dx, dy, dx, consecutive + 1))
    for ddy, ddx in ((0, -1), (0, 1), (-1, 0), (1, 0)):
        if ((y, x) == (0, 0) or consecutive >= 4) and (ddy, ddx) != (dy, dx) and (ddy, ddx) != (-dy, -dx) and 0 <= y + ddy < len(grid) and 0 <= x + ddx < len(grid[0]):
