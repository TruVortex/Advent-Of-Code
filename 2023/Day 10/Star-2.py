...
if grid[start[0]][start[1] - 1] in dirs['left'] and grid[start[0]][start[1] + 1] in dirs['right']:
    ...
    cur[1] += 1
cnt, vis, loop = 1, [[False] * len(grid[0]) for _ in range(len(grid))], {tuple(cur)}
...
while cur != start:
    ...
    loop.add(tuple(cur))
ans, grid = 0, [[grid[i][j] if (i, j) in loop else '.' for j in range(len(grid[0]))] for i in range(len(grid))]
